...
Validation: preview and production builds/TypeScript pass, lint passes, and all four SEO response checks pass in each environment (eight checks total). Tests inspect actual HTML robots/canonical metadata, host/API noindex headers, robots rules and sitemap contents. Search Console submission and actual Google indexing are not yet completed.
...
Validation: production build, lint and TypeScript pass; 35 SEO, compact-layout and hero checks pass. Desktop/mobile screenshots were inspected, including the new location line and project selection. Tests verify server-rendered profile data, title and all three project panels. Google rich-result validation and actual search rankings are not asserted.

## September 20 — page system

The compact homepage remains unchanged in length and now links to a matching inner-page system: a project index, static detail pages for OpsPilot AI, InsightLoop and the in-development CloudOps Knowledge Assistant, plus a journal landing and reusable article route. Shared typography, glass navigation, violet atmosphere, rounded surfaces and technical line forms preserve the approved visual language without duplicating the portrait WebGL scene. Case studies expose implementation evidence and explicit claim boundaries. The journal contains no manufactured articles; while empty it is noindex and omitted from the sitemap. Publishing a sourced, owner-reviewed entry should add it to the journal data, sitemap and indexable surface.

Validation: production Webpack build, TypeScript and lint pass. All 57 browser checks pass across the existing assistant, particle scene, navigation, SEO and new inner-page coverage. Desktop and mobile case-study/journal screenshots were inspected; layouts have no horizontal overflow. Turbopack could not bind its internal process port in the local sandbox, so the documented `--webpack` build fallback was used; this is an environment limitation, not an application build failure.

## September 22 — Google site name

The owner’s Google screenshot displays “Vercel” as the site-name label above the result, distinct from the page title. The preferred site name is “Agentic AI Developer”. The homepage now declares `WebSite` JSON-LD, and root metadata declares matching `og:site_name`. Visible editorial separators and assistant answers avoid em dashes. Google’s generated site name may change only after recrawling and remains under Google’s control. `01.png` is a local review artifact, not a public asset.
