  ...
  return (
    <div className="profile-chat">
      ...
      <div className="chat-log" ref={log} role="log" aria-label="Conversation with Ahmad.AI" aria-live="polite" aria-relevant="additions" tabIndex={0}>
        ...
        {messages.map((message, index) => <article key={index} className={`chat-message chat-${message.role}`}><span className="eyebrow">{message.role === "user" ? "YOU" : "AHMAD.AI"}{message.result?.mode === "profile" && " / PROFILE ANSWER"}</span><p>{message.content}</p>{message.result?.notice && <p className="chat-notice">{message.result.notice}</p>}{message.result && (message.result.links?.length ?? 0) > 0 && <div className="chat-sources" aria-label="Relevant links">{message.result.links?.map(source => <a key={source.id} href={source.href}>{source.title} <span aria-hidden="true">↗</span></a>)}</div>}</article>)}
        {busy && <p className="chat-pending" role="status">Finding an answer…</p>}
      </div>
      {!messages.length && <div className="chat-prompts" aria-label="Suggested questions">{prompts.map(prompt => <button key={prompt} type="button" disabled={busy} onClick={() => { void send(prompt); }}>{prompt}<span aria-hidden="true">↗</span></button>)}</div>}
